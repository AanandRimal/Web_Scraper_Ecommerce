    ...
    if len(search_quereies) >= 1:
        ...
        def Integrated():
            def Amazon():
                ...
                # Set up the Selenium WebDriver (You need to have the appropriate driver installed and in your PATH)
                driver = webdriver.Chrome()  # You can replace 'Chrome' with 'Firefox' or 'Edge' based on your preference
                time_variable = 2
                ...
                def link_scrape():
                    ...
                    outlines = soup.find_all("a", {"class": "a-link-normal s-no-outline"})

                    for outline in outlines:
                        link = outline.get("href")
                        Amazon_Link.append(link)
            ...
            def Snapdeal():
                ...
                def link_scrape():
                        time.sleep(time_variable)
                        links = data.select('.product-desc-rating')


                        for link in links:
                            link_identification = link.find_all('a', 'dp-widget-link noUdLine')
                            if link_identification:
                                link_name = link_identification[0].get("href")
                                Snapdeal_Link.append(link_name)
                ...
                #                 Snapdeal_Link.append(link_name)
                    ...
                    # outlines = data.find_all("a.dp-widget-
                    # outlines = data.find_all("a.dp-widget-link hashAdded")
                        #
                        #
                        # for outline in outlines:
                        #     link = outline.get("href")
                        #
                        #     Snapdeal_Link.append(link)
            ...
            def Flipkart():
                ...
                time.sleep(time_variable)
                try:
                    recievet = requests.get(URL, headers=headers, timeout=20)
                except Exception as e:
                    print("An error occurred:", str(e))

                soupp = BeautifulSoup(recievet.content, 'html.parser')
                ...
                def initial_viability_test():
                    test_count = 0
                    The_Whole_Page = soupp.prettify()
                    while test_count < 2:
                        print(The_Whole_Page)
                        test_count += 1
                        time.sleep(time_variable)

                first_set_product_types = ["mobile", "laptop", "washingmachine", "television", "mixer" ,"grinder" ,"iphone 13", "iphone 14", "Iphone", "Ipad", "ipad", "monitor", "camera", "iphone"]
                product_search=str(Product_search)
                def get_class_names(product_search):
                    if any(product_type in product_search.lower() for product_type in first_set_product_types):
                        return "_4rR01T", "_30jeq3 _1_WHN1", "_396cs4", "_1fQZEK"  # First set of class names
                    else:
                        return "IRpwTa", "_30jeq3", "_2r_T1I", "IRpwTa"  # New class names for other products

                def name_scrape():
                    time.sleep(time_variable)

                    name_class, _, _, _ = get_class_names(Product_search)
                    print(name_class)
                    outlines = soupp.find_all(class_=name_class)
                    print(outlines)
                ...
                def price_scrape():
                    ...
                    _, price_class, _, _ = get_class_names(Product_search)
                    print(price_class)
                    outlines = soupp.find_all("div", {"class": price_class})
                ...
                def image_scrape():
                    ...
                    _, _, image_class, _ = get_class_names(Product_search)
                    print(image_class)
                    outlines = soupp.find_all("img", {"class": image_class})
                ...
                def link_scrape():
                    ...
                    _, _, _, link_class = get_class_names(Product_search)
                    outlines = soupp.find_all(class_=link_class)

                    for outline in outlines:
                        link = outline.get("href")
                        Flipkart_Link.append(link)

                # def name_scrape():
                #     time.sleep(time_variable)
                #
                #     outlines = soupp.find_all("div", {"class": "_4rR01T"})
                #
                #
                #     for outline in outlines:
                #         name = outline.text
                ...
                #     time.sleep(time_variable)
                #
                #     outlines = soupp.findAll("div", {"class": "_30jeq3 _1_WHN1"})
                #
                #     for x in range(len(outlines)):
                #         outline = outlines[x]
                #         price = outline.text
                ...
                # def image_scrape():
                #     time.sleep(time_variable)
                #     outlines = soupp.findAll("img", {"class": "_396cs4"})
                #
                #     for x in outlines:
                #         image_link = x['src']
                #         Flipkart_ImageLinks.append(image_link)
                ...
                #     time.sleep(time_variable)
                #
                #     outlines = soupp.find_all("a", {"class": "_1fQZEK"})
                #
                #     for outline in outlines:
                #         link = outline.get("href")
                #         Flipkart_Link.append(link)
                ...
                #
                # try:
                #     recievet = requests.get(URL, headers=headers)
                #     recievet = recievet.text
                #
                # except:
                ...
                #     browser.close()
                #
                # soup = BeautifulSoup(recievet, 'lxml')
                #
                #
                # def initial_viability_test():
                #     test_count = 0
                ...
                # def name_scrape():
                #     time.sleep(time_variable)
                #     outlines = soup.findAll("div", {"class": "_4rR01T"})
                #
                #     for x in range(len(outlines)):
                #         outline = outlines[x]
        ...
        def SQL():
            def create_tables():
                ...
                try:
                    cursor.execute("CREATE TABLE snapdeal (name TEXT, price INTEGER(20), image TEXT, link TEXT)")
                except:
                    "Table already created"

                try:
                    cursor.execute("CREATE TABLE flipkart (name TEXT, price INTEGER(20), image TEXT,link TEXT)")
                except:
                    "Table already created"

                try:
                    cursor.execute("CREATE TABLE amazon (name TEXT, price INTEGER(20), image TEXT, link TEXT)")
                except:
                    "Table already created"
            ...
            def insert_data():
                ...
                def snapdeal():
                    sqlFormula = "INSERT INTO snapdeal (name, price, image,link) VALUES (%s, %s, %s,%s)"
                    data = []

                    for x in range(len(Snapdeal_Names)):
                        data_installment = (Snapdeal_Names[x], Snapdeal_Prices_2[x], str(Snapdeal_imagelinks[x]),Snapdeal_Link[x])
                ...
                def flipkart():
                    sqlFormula = "INSERT INTO flipkart (name, price, image,link) VALUES (%s, %s, %s,%s)"
                    data = []

                    for x in range(len(Flipkart_Names)):
                        data_instance = (Flipkart_Names[x], Flipkart_Prices_2[x], str(Flipkart_ImageLinks[x]),"https://www.flipkart.com" + Flipkart_Link[x])
                ...
                def amazon():
                    sqlFormula = "INSERT INTO amazon (name, price, image,link) VALUES (%s, %s, %s,%s)"
                    data = []

                    upper_limit = [len(Amazon_Prices_2), len(Amazon_ImageLinks), len(Amazon_Names),len(Amazon_Link)]
                    upper_limit.sort()

                    limit = min(upper_limit)

                    for x in range(limit):
                        data_instance = (Amazon_Names[x], Amazon_Prices_2[x], str(Amazon_ImageLinks[x]), "https://www.amazon.in" +Amazon_Link[x])
                        data.append(data_instance)
